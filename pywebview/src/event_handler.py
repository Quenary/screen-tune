...
class EventHandler:
    ...
    def start(self):
        self.stop()
        self._interval_subscription = combine_latest(
            interval(self._interval), self._config.observe_config()
        ).subscribe(lambda x: self._on_next_interval(x[1]))
        self._live_preview_subscription = combine_latest(
            self._live_preview_active, self._live_preview_values
        ).subscribe(lambda x: self._on_next_live_preview(x[0], x[1]))

    def _on_next_interval(self, config: ConfigDict):
        if config is None:
            return
        ...
        config_applications = config.get("applications", {})
        config_applications_entry = config_applications.get(
...
# TODO поправить лишние вызовы
# При смене окна вызов идет два раза
# При отсутствии окна в конфиге вызов все равно идет (с дефолтным конфигом?)
    def _set_values(
        self,
        displays: list,
